...
public class VanillaTweaks implements ModInitializer {
	...
	public void onInitialize() {
		...
		if (CONFIG.MORE_MOB_HEADS.isEnabled()) {
			List<String> keys = CONFIG.MORE_MOB_HEADS.getOdds().getKeys();
			for (int i = 0; i < keys.size(); i++) {
				Registry.register(Registry.BLOCK, new Identifier(MOD_ID, new Identifier(keys.get(i)).getPath() + "_head"), Register.GROUND_HEADS.get(i));
				Registry.register(Registry.BLOCK, new Identifier(MOD_ID, new Identifier(keys.get(i)).getPath() + "_wall_head"), Register.WALL_HEADS.get(i));
